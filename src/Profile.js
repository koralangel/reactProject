...
import './App.css';
import { doApiGetToken, doApiPostToken, doPostImgs, doApiGet } from './services/apiService';
...
function Profile() {
    ...
    useEffect(() => {
        ...
        let urlCity = 'http://localhost:3000/cities';
        Axios.get(urlCity)
            .then(res => {
        ...
        if (localStorage.getItem('tok')) {
            let url = "http://localhost:3000/users/single";
            doApiGetToken(url)
                .then(data => {
                    if (data.first_name) {
    ...
    const editImg = () => {
        ...
        if (imgUpStatus === 0) {
            ...
            for (let i = 0; i < userImgArr.length; i++) {
                ...
                div.style.backgroundImage = 'url(http://localhost:3000/images/users_imgs/' + userImgArr[i] + ')';
    ...
    const onDelAccount = () => {
        ...
        let urlA = "http://localhost:3000/msgs/deluserad";
        doApiPostToken(urlA)
            .then(data => {
                console.log(data)
                if (data.message) {
        ...
        let urlB = "http://localhost:3000/msgs/deluseradb";
        doApiPostToken(urlB)
            .then(data => {
                console.log(data)
                if (data.message) {
        ...
        let urlPost = "http://localhost:3000/ads/deluserad";
        ...
        let urlUser = "http://localhost:3000/users/deluser";
    ...
    const cancelBlock = (e) => {
        ...
        let url2 = "http://localhost:3000/users/profile/" + blockUserId;
        doApiGet(url2)
            .then(data => {
                if (data._id) {
                    let tIndex;
                    ...
                    let url3 = "http://localhost:3000/users/updateanother";
                                                    ...
                                                    {userData.block_users.map((user, i) => {
                                                        return (
                                                            <div style={{ marginBottom: "6px" }}>
                                                                ...
                                                                <img style={{ width: "34px", borderRadius: "50%", marginRight: "6px" }} src={'http://localhost:3000/images/users_imgs/' + user.image} />
                                    ...
                                    <div className="col-md-8 no-pad">
                                        <div style={{ cursor: "pointer", border: "4px dotted rgb(5, 0, 190)" }} className="user-image" data-toggle="modal" data-target="#myModal" onClick={editImg}>
                                            {userData.image === undefined ? null :
                                                <img className="img-responsive thumbnail" src={'http://localhost:3000/images/users_imgs/' + userImgArr[0]} />
