...
import { doApiPostToken, doApiGetToken, doApiGet } from '../services/apiService';
...
function AdList(props) {
    ...
    // $("#try-fade").slideLeft(100).slideRight(100);
    // $("#try-fade").fadeIn(5000);

    //const socket;
    // const socket = io(ENDPOINT);

    // let [userCat, setUserCat] = useState([]);  // מערך קטגוריות
    ...
    //setUserCat(props.userData.categories)

    let [userState, setUserState] = useState(null)


    // let hitsoty = useHistory();
    ...
    useEffect(() => {
        ...
        if (userState == null) {
            let urlAds = 'http://localhost:3000/ads';  // שולף את כל הפוסטים
            doApiGetToken(urlAds)
                .then(data => {
                    if (data.length === 0) {
                        ...
                        if (data[0]._id) {
                            let urlUser = "http://localhost:3000/users/single";
    ...
    const onClickPass = (event) => {
        ...
        let url = "http://localhost:3000/ads/update";  // שולח לשרת את העידכון
    ...
    const onClickLike = () => {
        ...
        let url1 = "http://localhost:3000/ads/update";
        ...
        if (checkIfInclude.length === 0) {
            ...
            let url2 = "http://localhost:3000/users/update";
            ...
            let url3 = "http://localhost:3000/users/updatelike";
            ...
            let urlGet = "http://localhost:3000/users/profile/" + idUserAd;
            doApiGet(urlGet)
                .then(data => {
                    if (data.first_name) {
                        console.log(data);
                        ...
                        if (checkIfMatch.length > 0) {
                            ...
                            document.getElementById("ownerImg").style.backgroundImage = 'url(http://localhost:3000/images/users_imgs/' + userState.image[0] + ')';
                            document.getElementById("secImg").style.backgroundImage = 'url(http://localhost:3000/images/users_imgs/' + presentAd.user.image[0] + ')';
                            ...
                            let urlMsg = "http://localhost:3000/msgs/add";
